...
Write-Host "=== LexHub: Server Dev Tests Watch (Docker Mongo + pnpm) ==="
...
try {
  ...
  Write-Host "🧪 Running server tests in watch mode (Ctrl+C to stop)..."
  pnpm --filter lexhub-server test:watch
}
finally {
