...
export default function ServicesSection() {
  const services = [
    {
      icon: <Scan className="h-12 w-12 text-primary-600" />,
      title: "סריקה ואחסון",
      description: "סריקה מקצועית ואחסון בענן מאובטח",
      features: ["סריקה", "ארגון", "אחסון בענן"]
    },
    {
      icon: <FolderOpen className="h-12 w-12 text-primary-600" />,
      title: "ארגון וסדר",
          ...
          {services.map((service, index) => (
            <Card key={index} className="card-hover">
              <CardHeader className="text-center">
                ...
                <CardTitle className="font-headline text-xl font-semibold text-gray-900">
                  {service.title}
                </CardTitle>
              </CardHeader>
              <CardContent>
